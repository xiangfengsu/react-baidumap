...
import {Map,Marker,Polygon} from '../../index';
...
<Playground>
    {()=>{
       ...
       return  <Map mapKey="SbtCslmFwveeKa1mM1dHoNL2uyambFMN"  enableScrollWheelZoom center={[121.359593,31.228016]} zoom={16} mapContainerStyle={{height:'400px'}}>
                 <Polygon points={points}  strokeColor="blue" strokeWeight={2} />
...
<Playground>
    {()=>{
       ...
       return  <Map mapKey="SbtCslmFwveeKa1mM1dHoNL2uyambFMN"  enableScrollWheelZoom center={[121.359593,31.228016]} zoom={16} mapContainerStyle={{height:'400px'}}>
                 <Polygon points={points} enableEditing={enableEditing}  strokeColor="blue" strokeWeight={2} />
...
| 参数 | 说明 | 参数 | 
| --- | --- | --- | 
| onClick | 点击多边形后会触发此事件| {type, target, point, pixel} |
| onDblClick | 双击多边形后会触发此事件 | {type, target, pixel, point} |
| onMouseDown | 鼠标在多边形上按下触发此事件 | {type, target, pixel, point} |
| onMouseUp | 鼠标在多边形释放触发此事件 | {type, target, pixel, point} |
| onMouseOut | 鼠标离开多边形时触发此事件 | {type, target, pixel, point} |
| onMouseOver | 当鼠标进入多边形区域时会触发此事件 | {type, target, pixel, point} |
| onRemove | 移除多边形时触发 | {type, target} |
